...
(function (root) {
  ...
  var MESSAGES_MAP = {
    ...
    war: function war(rowIdx) {
      return "Formated ".concat(rowIdx, " lines, abnormal JSON source!");
    },
    scc: function scc(rowIdx) {
      return "Success formated ".concat(rowIdx, " lines!");
    },
    val: function val(rowIdx) {
      return "Invalid value in line: ".concat(rowIdx);
    },
    ost: function ost(rowIdx) {
      return "Expect a string in line: ".concat(rowIdx);
    },
    col: function col(rowIdx) {
      return "Expect a colon in line: ".concat(rowIdx);
    },
    end: function end(rowIdx, brc) {
      return "Expect a comma or a \"".concat(brc, "\" in line: ").concat(rowIdx);
    }
  };
  ...
  function fmt2json(source, options) {
    ...
    try {
      try {
        if (fmtSource !== '') eval("fmtSource = ".concat(fmtSource));
    ...
    function commaHandler() {
      ...
      fmtResult += fmtOptions.isExpand ? ",".concat(BREAK + curIndent) : ',';
      chkFmtExpect(fmtSource[0]);
      setFmtExpect(fmtSource[0]);
      fmtSource = getSrcRest();
    ...
    function quoteNormalStr(qtStr, quote) {
      ...
      qtStr = isFromAbnormal ? qtStr.replace(/(?!\\[b|f|n|\\|r|t|x|v|'|"|0])\\/gm, '\\\\') : qtStr.replace(/\\/gm, '\\\\');
      ESCAPES_MAP.forEach(function (esp) {
        return qtStr = qtStr.replace(esp.ptn, esp.str);
      });
      var quote_ = isEscape ? "\\".concat(quote) : quote;
